...
7. For each sightseeing/activity stop, provide ONLY the activity name, a short description, the rough area/neighborhood in the city,
   a suggested time window, and an estimated cost in USD.
8. Do not repeat the same attraction/place on multiple days; distribute unique places across the full itinerary.
9. Do NOT include breakfast, lunch, or dinner stops. The system will add those as verified restaurant activities later.
...
1. When the request needs real place data, attractions, hotels, restaurants, or routes, call the relevant tool first.
2. Tool `photos` values may be compact image IDs, not full URLs.
3. Never invent image IDs or image URLs; only use values returned by tools.
4. Final API responses must resolve compact image IDs back to their original URLs before returning to the client.
"""
    @staticmethod
